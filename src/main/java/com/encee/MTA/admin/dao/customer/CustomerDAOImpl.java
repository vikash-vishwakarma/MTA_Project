package com.encee.MTA.admin.dao.customer;
...
public class CustomerDAOImpl implements CustomerDAO {
	...
	public CustomerDomain getCustomer(long id) throws Exception {
		try 
		{
			String hql = "From CustomerDomain where id=?1 and status=true";
	...
	public Response updateCustomer(CustomerDomain customer) throws Exception {
		...
		try {
			...
			customers.setCityId(customer.getCityId());
			customers.setStateId(customer.getStateId());
			...
			customers.setPassword(CommonUtils.encriptString(customer.getPassword()));
			 entityManager.flush();
			 response.setStatusText(StatusCode.SUCCESS.name());
		}
		catch(Exception e) {
	...
	{
		try {
			String hql="From CustomerDomain where status=true";
	...
	public Response deleteCustomer(long id) throws Exception {
		...
		try {
			...
			customerDomain.setStatus(false);
			entityManager.flush();
			response.setStatusText(StatusCode.SUCCESS.name());
		}
		catch(Exception ex) {
